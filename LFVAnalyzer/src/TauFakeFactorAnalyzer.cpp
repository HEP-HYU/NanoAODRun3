...
void TauFakeFactorAnalyzer::defineCuts() {
    ...
    std::string chargeName = _isMuonCh ? "mutau_charge" : "etau_charge";

    if (_mode == "lss") { // loose tau vs jet && SS
        addCuts(lepCut + " && PV_npvsGood > 0 && nloosetaupass == 1 && ncleantaupass == 0 && " + chargeName + " > 0", "0");
        ...
        addCuts(lepCut + " && PV_npvsGood > 0 && ncleantaupass == 1 && " + chargeName + " > 0", "0");
...
void TauFakeFactorAnalyzer::defineMoreVars() {
    ...
    // Define tau variables for the selected tau of the given mode:
    // For loose modes (lss, los), the selected tau is the loose tau (nloosetaupass == 1).
    // For tight modes (tss, tos), the selected tau is the tight tau (ncleantaupass == 1).
    if (_mode == "lss" || _mode == "los") {
        addVar({"Tau1_pt", "Tau_pt_loose[0]", ""});
        addVar({"Tau1_charge", "Tau_charge_loose[0]", ""});
        addVar({"Tau1_decayMode", "Tau_decayMode_loose[0]", ""});
        addVar({"Tau1_pt_gen", "(Tau_pt_loose_gen.size() > 0) ? float(Tau_pt_loose_gen[0]) : -1.0f", ""});
        addVar({"Tau1_decayMode_gen", "(Tau_pt_loose_gen.size() > 0) ? int(Tau_decayMode_loose[0]) : -1", ""});
    } else {
        addVar({"Tau1_pt", "Tau_pt[0]", ""});
        addVar({"Tau1_charge", "Tau_charge[0]", ""});
        addVar({"Tau1_decayMode", "Tau_decayMode[0]", ""});
        addVar({"Tau1_pt_gen", "(Tau_pt_gen.size() > 0) ? float(Tau_pt_gen[0]) : -1.0f", ""});
        addVar({"Tau1_decayMode_gen", "(Tau_pt_gen.size() > 0) ? int(Tau_decayMode[0]) : -1", ""});
    }
    ...
    // EventWeights (TauFakeFactorAnalyzer runs only with nosyst or data)
    if (_syst == "data") {
...
void TauFakeFactorAnalyzer::bookHists() {
    ...
    if (_mode == "lss" || _mode == "los") {
        add1DHist({"h_ncleanjetspass", ";Jet multiplicity;Events", 10, 0.0, 10.0}, "ncleanjetsloosepass", "eventWeight", "", "0", maxstep);
        add1DHist({"h_ncleanbjetspass", ";b-tagged jet multiplicity;Events", 5, 0.0, 5.0}, "ncleanbjetsloosepass", "eventWeight", "", "0", maxstep);
