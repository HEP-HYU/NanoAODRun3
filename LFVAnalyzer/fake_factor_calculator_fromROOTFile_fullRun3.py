...
for bin_edge in bin_edges:
    ...
    for year in year_list:
        for region, nevents in nevents_region.items():
            ...
            nevents_region[region]['gentau_total'] += ufloat(gen_int, gen_err.value)
            nevents_region[region]['mc_total']     += ufloat(all_int, all_err.value)
            nevents_region[region]['ndata']        += int(data_int)
            plot_file.Close()
    ...
    print('        map_ff["Run3"]["nom"]  = {0:.4g};'.format(SF.n))
    print('        map_ff["Run3"]["stat"] = {0:.4g};'.format(SF.std_dev/SF.n))
    print('        map_ff["Run3"]["syst"] = {0:.4g};'.format(SF_closure.n))
