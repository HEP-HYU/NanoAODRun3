...
#ifndef ttHHAnalyzer_H_
#define ttHHAnalyzer_H_

#include "NanoAODAnalyzerrdframe.h"

class ttHHAnalyzer: public NanoAODAnalyzerrdframe {

public:
    ttHHAnalyzer(TTree *t, std::string outfilename, std::string year="", std::string ch="", std::string syst="", std::string jsonfname="", bool applytauFF=false, string globaltag="", int nthreads=1);
    void defineCuts();
...
private:
    ...
    struct tauFFfunctor {
        ...
        double operator()(floats &tau_pt_, floats &tau_gen_pt_, ints &tau_dm_) {
           return ttHHAnalyzer::tauFF(year_, unc_, direction_, tau_pt_, tau_gen_pt_, tau_dm_);
        }
    };
};

#endif /* ttHHAnalyzer_H_ */
